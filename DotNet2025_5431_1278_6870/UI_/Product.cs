...
namespace UI
{
    public partial class Product : Form
    {
        ...
        {
            products = s_bl.Product.ReadAll().Where(p => p.ProductName.Contains(findProductTxt.Text) &&
                                                        p.Category == (Categories)Enum.Parse(typeof(Categories), saerchCategoryCb.Text)).ToList();
            productDgv.Rows.Clear();
            foreach (BO.Product product in products)
            {
                productDgv.Rows.Add(product.ProductCode, product.ProductName, product.Quantity, product.Price, product.Category);
